...
$name = ( false == $item_params->get('djmegamenu-hide-title', 0) ) ? '<span class="name">' . $item->title . '</span>' : '';
$subtitle = '';
...
if ($params->get('icons') > 0 && $params->get('icons') < 3) {
	...
	if ( !empty($faicon) ) {
		if ( $item_params->get('menu_text', 1) ) {
			...
			$icon = '<span class="dj-icon ' . $faicon . '" aria-hidden="true" title="' . htmlspecialchars($item->title) . '"></span>';
			$title_attr .= 'aria-label="' . htmlspecialchars($item->title) . '" ';
		}
		if ($item->level == $startLevel) $aclass .= ' withicon';
	} else if ( $item->menu_image ) { //images
		$icon = '<img class="dj-icon" src="' . $item->menu_image . '" alt="' . $item->title . '" />';
		if ($item->level == $startLevel) $aclass .= ' withimage';
	}
}

$itemclass = ( !empty($icon) ) ? 'image-title' : 'title';
$linktype = $icon . '<span class="' . $itemclass . '">' . $name . $badge . $subtitle . '</span>';
