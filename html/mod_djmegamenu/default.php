...
$class_attr[] = 'dj-fa-' . $params->get('fa');
	...
	<ul id="dj-megamenu<?php echo $module->id; ?>" class="dj-megamenu <?php echo $class_attr; ?>" <?php echo !empty($options) ? "data-options='" . $options . "'" : ""; ?> data-trigger="<?php echo (int)$params->get('width') ?>" role="menubar" aria-label="<?php echo $module->title; ?>">
		...
		foreach ($list as $index => &$item) :
			...
			if ($first) {
				...
				echo '<div class="dj-subcol" style="width:' . $item_params->get('djmegamenu-column_width') . '"><ul class="dj-submenu" role="menu">';
				$class .= ' first';
			}
			...
			if ($item->parent && $item->level == $startLevel && $item_params->get('djmegamenu-fullwidth')) {
				$class .= ' fullsub';
			}
			...
			if ($item->deeper) {
				...
				if ($item->level > $startLevel && $expand[$item->id] == 'tree') {
					echo '<ul class="dj-subtree" role="menu">';
				} else {

					if ($item_params->get('djmegamenu-fullwidth')) {
						$style = 'width: 100%;';
						$style_in = 'width: 100%;';
						$open_dir = '';
					} else {
					...
					echo '<div class="dj-subcol" style="width:' . $item_params->get('djmegamenu-first_column_width') . '"><ul class="dj-submenu" role="menu">';
				}
				$first = true;
			}
